//accumulate 함수와 round 함수 차이점 알아보기, 최빈값 정렬 복습
...
using namespace std; // 자주 사용하는 네임공간 선언

typedef pair<int, int> ci;  // pair<int, int>에 ci라는 별칭 부여

// 비교함수
bool cmp(const ci &a, const ci &b) {
    if (a.second == b.second) { //두 번째 정수값이 같다면
        return a.first < b.first; // 첫 번째 값에 의한 비교
    }
    return a.second > b.second; //아니라면 두 번째 값에 의한 비교
}
...
int findMode(int n, vector<int> &arr) { //
    ...
    for (int i = 1; i < n; i++) {
        // 만약 직전 값과 같은 값이라면
        if (arr[i] == arr[i - 1]) {
            count[current_idx].second++; //개수 증가
        } else {
    ...
    if (count[0].second == count[1].second) {
        return count[1].first; //최빈값이 여러 개이면 2번째로 작은 값 반환
    }
    return count[0].first; //최빈값 1개일 때 반환
}
